...
/*  JSON code courtesy of Florian Sesser <fs@it-agenten.com>, modified for extjs(ext js and jquery)
[
  {"type": "directory", "name": "name", "mode": "0777", "user": "user", "group": "group", "inode": ###, "dev": ####, "time": "00:00 00-00-0000", "children": [
    {"type": "link", "name": "name", leaf: true, "target": "name", "children": [... if link is followed, otherwise this is empty.]}
    {"type": "file", "name": "name", leaf: true, "mode": "0777", "size": ###, "group": "group", "inode": ###, "dev": ###, "time": "00:00 00-00-0000"}
...
off_t extjs_listdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  ...
  while(*dir) {
    if (!noindent) extjs_indent(lev);

    if ((*dir)->lnk) mt = (*dir)->mode & S_IFMT;
    else mt = (*dir)->mode & S_IFMT;
    for(t=0;ifmt[t];t++)
      if (ifmt[t] == mt) break;
    fprintf(outfile,"{\"type\":\"%s\"%s", ftype[t], strcmp(ftype[t], "directory")?",leaf:true":"");
...
off_t extjs_rlistdir(char *d, int *dt, int *ft, u_long lev, dev_t dev)
{
  ...
  extjsr_listdir(dir, d, dt, ft, lev);
...
void extjsr_listdir(struct _info **dir, char *d, int *dt, int *ft, u_long lev)
{
  ...
  while(*dir) {
	...
	if (!strcmp(d,"/")) sprintf(path,"%s%s",d,(*dir)->name);
      ...
      extjsr_listdir((*dir)->child, fflag? path : NULL, dt, ft, lev+1);
      nlf = TRUE;
      *dt += 1;
    } else {
      if ((*dir)->isdir) *dt += 1;
